...
class Shortcode {
    ...
    private function render_shortcode( $type_key, $template_file ) {
        ...
        if ( isset($_POST['np_save']) && check_admin_referer('np_save_prefs', 'np_nonce') ) {
            ...
            // PREVIOUS VALUES
            // --------------------------------
            $previous_types = $saved[$type_key]['listing']   ?? [];
            $previous_locs  = $saved[$type_key]['locations'] ?? [];

            // --------------------------------
            // REMOVE UNCHECKED TERMS
            // --------------------------------
            remove_user_from_terms(
                array_diff($previous_types, $selected_types),
                $user_id
            );

            remove_user_from_terms(
                array_diff($previous_locs, $selected_locations),
                $user_id
            );
        ...
        // $dns_notify_prefs = get_user_meta( $user_id, 'dns_notify_prefs', true );

        // Messages::pri( $dns_notify_prefs );
